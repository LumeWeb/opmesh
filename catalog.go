// Registry, policy enforcement, and input normalization for opmesh.
package opmesh
...
// registry uses it to enforce Interaction, Visibility, and Safety at dispatch
...
const (
	...
	ActorApp
	// ActorHuman is a human operator.
	ActorHuman
)

// ToolDescriptor is the transport-neutral description of one operation that
// every frontend derives from. It is produced by Catalog.Describe (the
// discovery view). It carries only declared, non-executable metadata (Safety,
// Interaction, Visibility) plus the description and JSON-Schema inputs. It
// carries no Handler: dispatch always goes through the owning Catalog.Invoke,
// the single enforcement point for Interaction, Visibility, and Safety gates.
...
// Catalog is the registry every frontend consumes. It provides discovery
// (Search/Get/Describe) and dispatch (Invoke) with interaction, visibility,
// and safety enforcement built in.
type Catalog interface {
	// Add registers an operation. It returns an error wrapping
	// ErrDuplicateOperation if a different operation with the same name is
	// already registered.
	...
	// to the acting actor. Describe applies the same visibility boundary as
	// Search, so an app-only operation is invisible to model actors and a
	// model-only operation is invisible to app/human actors. An op excluded
	// from the caller's surface returns not-found, matching Search and Invoke.
...
// catalogImpl is the concrete registry backing the Catalog interface. Registry
// state is instance-scoped: there is deliberately no package-global map.
...
func (c *catalogImpl) Add(op Operation) error {
	if op == nil {
		return fmt.Errorf("opmesh: cannot add a nil operation")
	}
	if op.Name() == "" {
		return fmt.Errorf("opmesh: operation must have a name")
	}
	...
	if _, dup := c.ops[op.Name()]; dup {
		return fmt.Errorf("%w: %q", ErrDuplicateOperation, op.Name())
	}
	c.ops[op.Name()] = op
	return nil
}
...
func validateOperation(op Operation) error {
	for _, a := range op.Args() {
		...
		if a.AgentConfirm && (a.Name != "confirm" || a.Type != ArgTypeBool) {
			return fmt.Errorf("opmesh: operation %q arg %q: AgentConfirm requires a bool arg named \"confirm\"", op.Name(), a.Name)
		}
		...
		if len(a.Enum) > 0 && a.Type != ArgTypeString {
			return fmt.Errorf("opmesh: operation %q arg %q: enum declared on non-string arg type", op.Name(), a.Name)
		}
		if a.Default != "" && len(a.Enum) > 0 && !slices.Contains(a.Enum, a.Default) {
			return fmt.Errorf("opmesh: operation %q arg %q: default %q not in enum %v", op.Name(), a.Name, a.Default, a.Enum)
		}
		if a.Default == "" {
			continue
		}
		if err := validateDefault(a); err != nil {
			return fmt.Errorf("opmesh: operation %q arg %q: %w", op.Name(), a.Name, err)
		}
	}
	return nil
}
...
func (c *catalogImpl) Search(query, category string, v Visibility) []Operation {
	...
	// Map iteration order is nondeterministic; sort so frontend listings and
	// discovery output are stable across runs.
...
// excludes app-only ops; the app (VisibilityAppOnly) sees both app-only and
...
func (c *catalogImpl) Describe(name string, actor Actor) (ToolDescriptor, bool) {
	...
	// audience, so Describe and the frontends (which Search by surface) agree on
	...
	// The description is the single declared Description; audience-specific
	// presentation is an adapter concern, not registry state.
...
// Search. A model agent gets the agent-facing surface; the app and human
// operator get the app-facing surface.
...
func (c *catalogImpl) Invoke(ctx context.Context, name string, input map[string]any, actor Actor) (any, error) {
	...
	// the op runs headless. Every other destructive op — no confirm arg, an
	// AgentRequired confirm that only a human sets, or a confirm with a
	// different semantic — still returns the ErrConfirmRequired sentinel so the
	...
	h := op.Handler()
	if h == nil {
		return nil, fmt.Errorf("opmesh: operation %q has no handler", name)
	}
	// Required-arg validation (firstMissingRequiredArg): an arg is mandatory
	// only when Required AND has no Default (the default satisfies it via
	// NormalizeOperationInput below), and a present-but-empty or nil
	// string/slice counts as missing too, so every surface rejects the same
	// inputs before a Handler runs.
...
// (a human-only contract), a different semantic, or a human hand-off field —
// is deliberately excluded, so the model gate keeps refusing those ops with
...
// declared arg shapes and fills declared defaults, using the single resolveArg
// state machine. It is the standalone entry point callers that invoke an
// Operation.Handler directly (routing around Catalog.Invoke) must use so
// aliasing, coercion, defaults, and selection-group enforcement apply
...
func NormalizeOperationInput(op Operation, input map[string]any) (map[string]any, error) {
	...
	// Strip reserved plumbing keys that no handler reads, so they are
	...
	// stripped: per-invocation service selectors (e.g. an auth-token override
	// that falls back to config when absent) read it from the normalized input
...
// key from an operation that declares an arg with the same literal name, so
// the declared arg always takes precedence and is never stripped as transport
// plumbing.
...
func unknownOperationArg(args []OperationArg, input map[string]any) string {
	...
	for _, r := range reservedInputKeys() {
		known[r] = struct{}{}
	}
	for k := range input {
		if _, ok := known[k]; !ok {
...
// reservedInputKeys returns the frozen set of input-map keys that plumbing
// layers inject into an operation's input but that are not declared
...
// It is a function returning a freshly-built literal rather than a
// package-level mutable map: opmesh owns no mutable package state, so callers
// cannot mutate the shared set.
//
// Only non-registry flows emit extra transport plumbing; a client-forged key
// is an unrecognized argument and SHOULD be rejected.
func reservedInputKeys() []string {
	return []string{ReservedAuthTokenKey, ReservedRequestStateKey}
}

// ReservedAuthTokenKey is the shared name of the reserved auth-token override
// input key. It lives here (the transport-agnostic layer) so operation
// packages can reference it without an import cycle.
const ReservedAuthTokenKey = "auth_token"

// ReservedRequestStateKey is the reserved input key a transport layer may
// inject on a retried operation call carrying a client-echoed request-state
// token, used to recover cross-round session context after an
...
// classification.
//
// The input is `any` (Handler.Execute takes map[string]any), so a generic
// resolveArg[T] cannot bind T statically at the call sites. An explicit type
// switch is therefore required here.
func resolveArg(a OperationArg, raw any, present bool) (value any, st argState, err error) {
	...
	case ArgTypeString:
		...
		// caller passing an out-of-enum value gets an error, not a silent
		// invalid value. The match is case-insensitive so a value whose
		// semantic is case-insensitive by definition (e.g. a record type:
		// "txt" == "TXT") passes the gate and the handler normalizes the
		// casing before it reaches the wire. The returned value keeps the
		// caller's original casing; normalization is the handler's job.
		if len(a.Enum) > 0 {
		...
		return s, stateFilled, nil
	case ArgTypeRawJSON:
		// Pass-through: the value arrives as-is (a JSON string, or a decoded
		// array/object/scalar). The Handler is responsible for parsing it into
		// its domain shape. No coercion, no enum gate — the schema language is
		// entirely the Handler's.
	...
	case ArgTypeFlexibleID:
		// A string-or-integer id. Accept a string as-is; accept any
		// JSON/native integer by rendering its decimal form so a
		// StrArg/StrFlexibleArg reader always sees a string. This bridges the
		// drift where the backend id is an int while the arg the handler reads
		// is a string.
		switch v := raw.(type) {
		case string:
			if v == "" {
	...
	case ArgTypeInt:
		// A caller's int may arrive as a native integer, json.Number, or (via
		// JSON decoding) float64. Normalize every accepted shape to int64,
		// then apply the platform-int round-trip guard below so a value that
		// fits int64 but not this build's native int (e.g. 5e9 on 32-bit) is
		...
		case json.Number:
			...
			i64 = i
		case float64:
			// JSON has no integer type, so an int may arrive as float64.
			// Reject fractional and out-of-int64-range values instead of
			// truncating (30.7 -> 30) or silently overflowing.
			//
	...
	case ArgTypeNullableInt:
		...
		// distinguish an omitted arg (nil) from an explicit 0.
		switch n := raw.(type) {
		case *int:
	...
	case ArgTypeFloat:
		// The value may arrive as a native float, a native integer, or
		// json.Number. Widen every accepted shape to float64.
		switch n := raw.(type) {
		case float64:
			return n, stateFilled, nil
		case float32:
	...
	case ArgTypeDuration:
		// Accept a native time.Duration or a duration string.
		switch d := raw.(type) {
		case time.Duration:
...
// passes through; []any (the JSON-decoded shape) is converted element-wise,
// erroring on a non-string element rather than dropping it. nil is handled by
...
// and the JSON-Schema builder derive requiredness from this one predicate.
func isRequiredArg(a OperationArg) bool {
	return a.Required && a.Default == ""
}

// requiredArgNames lists the mandatory args in declaration order. It backs
// both the JSON-Schema "required" array (inputSchemaFromArgs) and any frontend
// that needs the set of required flags, keeping the schema and dispatch
// consistent on which args are optional.
...
// It is the enforcement point shared by Invoke and any frontend that skips
// Invoke, so every surface rejects the same inputs before a Handler runs.
// Returns nil when every required arg is satisfied. Classification comes from
// resolveArg, the same function normalizeInputDefaults uses.
...
// both the declared (kebab) name and its camelCase alias, so a caller sending
...
// "deviceName"). Used to accept the camelCase spelling of an arg a caller may
...
// identical, correctly-typed input regardless of which surface dispatched. A
// []string, a JSON-decoded []any, a JSON null, and an explicit []any{} all
// converge on []string, so a Handler type-asserting []string never panics, and
// a supplied non-empty value is never clobbered by a default.
func normalizeInputDefaults(args []OperationArg, input map[string]any) (map[string]any, error) {
	...
	for _, a := range args {
		...
		// camelCase alias (e.g. "deviceName" for "device-name"): callers
...
// violation surfaced as ErrSelector: the ambiguity direction a frontend cannot
// safely resolve (e.g. a remove op given both its cids and all=true, where the
// handler would otherwise silently remove everything). The empty direction
...
// erroring (validation at Add already rejects unparseable defaults).
func defaultValue(a OperationArg) any {
	...
	case ArgTypeDuration:
		...
		return d
	case ArgTypeStringSlice:
		// Default strings split on commas (SliceFlagSeparator style), so a
		// --tags a,b convention reaches the Handler as ["a","b"]. Split the
		// declared default the same way to keep the default and explicit paths
...
func inputSchemaFromArgs(name string, args []OperationArg) json.RawMessage {
	...
	for _, a := range args {
		...
		// several model-gateway validators) reject the tool at registration
		// with "array type must have items". Only ArgTypeStringSlice maps to
		// "array" today.
		if a.Type == ArgTypeStringSlice {
			p["items"] = map[string]any{"type": "string"}
		}
		// Advertise allowed values for an enum-constrained arg, so a caller
		// gets the guidance instead of guessing (which would only be rejected
		// at dispatch).
		if len(a.Enum) > 0 {
		...
		// Carry the arg description through the schema as the property
		// description. There is a single Help description; audience-tuned prose
		// is an adapter concern composed elsewhere.
		if a.Help != "" {
			p["description"] = a.Help
		}
		// Carry the sensitive marker through the schema so the adapter layer
		// can redact a secret arg's value from logs and echoed calls. Without
		// this, the arg would be indistinguishable from a normal one on the
		// agent surface.
		if a.Sensitive {
			p[SensitiveSchemaKey] = true
		}
	...
	// (isRequiredArg). An arg that is Required but declares a Default is
	// satisfied by the default and so is not required from the caller's
	// perspective. NOTE: AgentRequired is deliberately excluded — it is an
	// agent-surface-only requiredness that agent dispatch layers enforce
	// themselves, so it never leaks into a generic invocation path.
	...
	// Copilot, and other model gateways) implement only a subset of JSON
...
func jsonType(t ArgType) any {
	...
	case ArgTypeStringSlice:
		return "array"
	case ArgTypeRawJSON:
		// The JSON type is "any": the resolved value may be a string or a
		// decoded array/object. The paired RawSchema advertises the concrete
		// structured shape.
		return []string{"string", "array", "object"}
	case ArgTypeFlexibleID:
		// Accept a string or integer id so a caller can pass either the
		// integer form a list op emits or a string id/name.
...
// must be redacted (logs, echoed tool calls) on the agent surface. It matches
// OperationArg.Sensitive. Marking it in the shared schema rather than as a
// magic string literal keeps the marker strongly typed and consistent across
// the schema builder and Describe.

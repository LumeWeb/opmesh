...
// frontend positional binding relies on it instead of re-interpreting the
// string ad hoc.
...
// MapPositionalArgs maps a surface-supplied positional argument list onto the
...
// frontend, keeping the arg-position contract in the framework rather than
// hand-parsed per frontend.
//
...
// per-command validation (a destructive remove op given two positionals when it
// takes one must not operate on the first while ignoring the second).
func MapPositionalArgs(args []OperationArg, pos string, supplied []string, input map[string]any) error {
	...
	for i := start; i < len(names); i++ {
		...
		// reject it instead of silently preferring one.
...
// placeholder names no declared arg (a user-facing label that actually drives a
// differently-named arg), it falls back to the first string arg so the

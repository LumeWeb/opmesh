...
// ("cids") and a Bool ("all", default "false"), plus an unrelated required arg
// so required-ness doesn't mask the selector gate.
...
// TestSelectorGroupViaNormalizeOperationInput confirms direct callers that
// reuse NormalizeOperationInput (routing around Invoke) get the same gate.

...
// List is the normalized list cursor every *-list operation receives after a
// surface resolves its page/page-size args. Start is the 0-based offset into
// the full result set and Limit is the page size; backends express this as a
// start/end window (end = start + limit).
type List struct {
	Start int
	Limit int
}
...
// page-size. It matches common server-side list default windows so an
// un-paginated list degrades gracefully instead of hammering the backend.
const defaultPageSize = 10

// ListArgs returns the canonical paging arguments every *-list operation
// embeds so that all list surfaces expose the same human-friendly page
// protocol. Callers page with a 1-based page number and a page size; the
// surface wiring computes the underlying start/limit cursor (see
// ParseList/ParseListPage) so no caller ever has to reason about raw offsets.
...
// given positive default, so the derived Limit is always > 0. Server-side paged
// lists use this form: a positive Limit keeps the backend's exclusive
// end = start + limit strictly greater than start, satisfying the window
// invariant "end must be greater than start", and a bare `page N` advances by
...
// struct (e.g. domain/status/type filters) and is never touched by paging.
// This is the single shared options type: services alias it with their own
// filter struct instead of defining a bespoke options type.

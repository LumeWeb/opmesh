...
// idOp is a minimal op with a single FlexibleID "id" arg, mirroring how a
// get/delete operation declares a string id sourced from a list operation's
// integer ids.
func idOp() Operation {
...
// TestFlexibleIDAcceptsIntegerThroughNormalize is the regression guard for a
// list operation emitting the id as a JSON integer while the get/delete id arg
// was typed ArgTypeString: the normalizer would reject the integer with
...
// TestFlexibleIDAcceptsString preserves the string form (e.g. a caller passing
...
// ids above 2^53. JSON decoding with UseNumber means a large JSON integer id
// arrives as json.Number (exact), which the normalizer must convert losslessly
// instead of losing the low bits through float64. A silent float64 round-trip
// would produce a WRONG id, which for a delete operation could delete the wrong
// record.
...
// TestFlexibleIDSchemaAdvertisesStringAndInteger locks the JSON Schema
// contract: the id property must admit both a string and an integer so a caller
// can pass either the integer form a list operation emits or a string id/name.

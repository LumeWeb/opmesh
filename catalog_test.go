...
func stub(name, title, summary, desc, category string, safety Safety, inter Interaction, vis Visibility, args ...OperationArg) Operation {
	if len(args) == 0 {
...
func newTestCatalog(t *testing.T) Catalog {
	...
	ops := []Operation{
		...
		stub("pin add", "Add Pin", "pin a cid", "Long pin add desc",
		...
		stub("pin remove", "Remove Pin", "remove a pin", "Long pin remove desc",
		...
		stub("websites list", "List Websites", "list websites", "Long websites desc",
...
func TestAddRejectsDuplicate(t *testing.T) {
	...
	if !errors.Is(err, ErrDuplicateOperation) {
		t.Fatalf("err = %v, want errors.Is(err, ErrDuplicateOperation)", err)
	}
	if !strings.HasPrefix(err.Error(), "opmesh:") {
		t.Fatalf("error %q should carry the opmesh: prefix", err)
	}
}
...
func TestAddRejectsNilAndEmptyName(t *testing.T) {
	...
	} else if !strings.HasPrefix(err.Error(), "opmesh:") {
		t.Fatalf("error %q should carry the opmesh: prefix", err)
	}
...
// TestSearchDeterministicOrder pins that Search must sort by Name before
// returning — otherwise frontend listings and discovery output would vary
// between runs.
...
func TestDescribe(t *testing.T) {
	...
	// The description is the single declared Description; there is no
	// audience-specific variant in the registry.
	if sch.Description != "Long pin add desc" {
		t.Fatalf("Describe Description = %q, want the declared description", sch.Description)
	}
...
// TestDescribeScopesToActor verifies Describe enforces the app-only visibility
// boundary for model actors.
func TestDescribeScopesToActor(t *testing.T) {
	c := newTestCatalog(t)
	// A model actor describes a model-visible op.
	sch, ok := c.Describe("pin add", ActorModel)
	if !ok || sch.Description != "Long pin add desc" {
		t.Fatalf("model Describe = %+v ok=%v, want declared description", sch, ok)
	}
	// An app/human actor cannot describe a model-only op: the app surface
	// (VisibilityAppOnly) excludes it, mirroring what the app surface Search
	// returns — so Describe and the frontend can never disagree.
	...
	// VisibilityBoth ops are visible to both the model and app surfaces, with
	// the same declared description for every actor.
	if sch, ok := c.Describe("websites list", ActorModel); !ok || sch.Description != "Long websites desc" {
		t.Fatalf("model Describe(both) = %+v ok=%v, want declared description", sch, ok)
	}
	if sch, ok := c.Describe("websites list", ActorApp); !ok || sch.Description != "Long websites desc" {
		t.Fatalf("app Describe(both) = %+v ok=%v, want declared description", sch, ok)
	}
}
...
// TestInvokeHumanRequiredSentinel is a regression test for the exported
// ErrHumanRequired sentinel: an InteractionHumanOnly op invoked by a non-human
// actor (this is a Handoff-capable flow, the same gate refuses both
...
// has a different semantic or that must always route through the human
// hand-off: without the explicit AgentConfirm opt-in, the destructive gate
// keeps requiring a human.
...
// AgentRequired confirm (agent-surface "only a human sets this" contract) is
// refused for a model even when the model passes confirm=true.
...
func TestInvokeUnknownOperation(t *testing.T) {
	c := newTestCatalog(t)
	_, err := c.Invoke(context.Background(), "nope", map[string]any{}, ActorModel)
	if err == nil {
		t.Fatal("Invoke(unknown) should error")
	}
	if !errors.Is(err, ErrUnknownOperation) {
		t.Fatalf("err = %v, want errors.Is(err, ErrUnknownOperation)", err)
	}
	if !strings.HasPrefix(err.Error(), "opmesh:") {
		t.Fatalf("error %q should carry the opmesh: prefix", err)
	}
...
// absent args before dispatching to the Handler, so the input is uniform
// regardless of which surface dispatched (all surfaces rely on the same
// normalizeInputDefaults sink).
func TestInvokeAppliesDeclaredDefaults(t *testing.T) {
	...
	// Comma-joined slice defaults split on commas (--tags a,b => ["a","b"]
	// convention), trimming whitespace and dropping empties.
...
func TestInvokeAliasesCamelCaseToKebabArg(t *testing.T) {
	...
	// Caller sends camelCase only; the handler must see the canonical key.
...
// TestInvokeMissingRequiredArgErrors verifies Invoke rejects a dispatch when a
// required arg (no Default) is absent from input. An arg that is Required but
// declares a Default is satisfied by normalizeInputDefaults, so it must not be
// rejected.
func TestInvokeMissingRequiredArgErrors(t *testing.T) {
	...
	// Present-but-empty required string arg is treated as missing.
	...
	// A nil value (JSON null) is treated as empty/missing too.
...
// TestInvokeStringSliceAnyEmptyHandling pins that a caller decoding JSON arrays
// into []any (not []string) is handled: an empty []any{} must be treated like an
// empty []string by the required-arg check, and a defaulted StringSlice arg
// passed as []any{} must still receive its declared default (not a dangling
// empty []any{}).
func TestInvokeStringSliceAnyEmptyHandling(t *testing.T) {
	...
	// []string so a Handler type-asserting []string never panics (all surfaces
	// must converge on []string).
...
// TestInvokeStringSliceRejectsNonStringElement pins that a StringSlice supplied
// as []any with a non-string element (or as a non-slice scalar) must ERROR
// loudly rather than silently drop the element and hand the Handler a
// truncated value.
...
// TestInvokeOptionalEmptySliceReachesHandlerCoerced pins that an OPTIONAL
// StringSlice arg with no Default, supplied as the present-but-empty []any{},
// must reach the Handler as []string{} — never as the raw []any{}, which would
// panic a Handler type-asserting `.([]string)`.
...
// TestInvokeOptionalNullSliceReachesHandlerCoerced pins that an OPTIONAL
// StringSlice supplied as JSON null (nil) must not hard-error in the
// normalization path — it is coerced to []string{} (the []any{} shape), so a
// Handler type-asserting `.([]string)` still receives a []string.
...
// TestInvokeOptionalAbsentArgReachesZeroShape pins that an optional arg OMITTED
// from the input (stateAbsent) with no default must reach the Handler as the
// declared zero-shape (e.g. "" for a string, []string{} for a slice) — never as
// a naked nil, which would panic a Handler type-asserting `.(string)`.
...
// TestInvokeCoercesJSONScalarTypes pins that a caller decoding JSON scalars
// (integers -> float64, durations -> string) is coerced to the declared Go
// shapes so Handlers type-asserting int or time.Duration don't panic.
func TestInvokeCoercesJSONScalarTypes(t *testing.T) {
	h := &captureHandler{}
	c := NewCatalog()
	if err := c.Add(NewOperation(OperationSpec{
	...
	// JSON-decode shapes: int -> float64, float -> float64, duration -> string.
	...
	// A caller that already supplies the native shape passes through unchanged.
	h2 := &captureHandler{}
	if err := c.Add(NewOperation(OperationSpec{
...
// TestInvokeRefusesNeedsHandoffForNonHuman pins the gate: an operation with
// InteractionNeedsHandoff (external browser/device, two-call split) must be
...
// TestAddRejectsMalformedDefault pins that an operation whose typed arg
...
// TestResolveArgStateMachine pins that resolveArg is the SOLE classifier +
// coercer of a raw argument value. Absent/Null/Empty/Filled/Invalid must be an
// exhaustive partition, and the coercion it returns must agree with the state
// it reports. No new surface — resolveArg is unexported.
func TestResolveArgStateMachine(t *testing.T) {
	...
	cases := []struct {
		...
		// (resolveArg round-trips through the native int). The expected state is
		// therefore platform-dependent.
		{"int large platform-range", num, math.Exp2(62), true, largeIntWant},
		// The public Invoke API must accept any native Go integer a caller can
		// pass for an int arg — not just int and float64.
...
// TestInvokeRejectsWrongScalarType pins the strict-scalar behavior: a present
// arg of the wrong scalar type (e.g. a string where an int is declared) errors
// loudly in resolveArg instead of being passed through to the Handler as a
// wrong shape.
...
// TestInvokeAndSchemaHonorEnum pins that an enum-constrained string arg rejects
// out-of-range values at dispatch (not handed to the Handler), and the JSON
// Schema advertises the allowed values and the arg description so callers get
// guidance.
func TestInvokeAndSchemaHonorEnum(t *testing.T) {
	...
	// The schema advertises the enum array and the Help description.
	sch, ok := c.Describe("dns.add", ActorModel)
	if !ok {
		t.Fatal("Describe not found")
	}
	...
	if p["description"] != "record type" {
		t.Fatalf("schema description wrong, got %#v", p["description"])
	}
}

// TestInvokeVisibilityBoundarySymmetric pins that Invoke's visibility gate must
// mirror Search/Describe on BOTH sides: an op a surface cannot discover is also
// not executable for that surface's actor. The symmetric gate
// (visibleTo(op.Visibility(), actorVisibility(actor))) refuses whichever
// surface the op is excluded from.
func TestInvokeVisibilityBoundarySymmetric(t *testing.T) {
	...
	// Model-only is invisible to the app/human surface -> refused for both, and
	// absent from the app search surface.
...
// TestAddRejectsBadEnumConfig pins that an Enum declared on a non-string arg,
// or a Default that is not a member of the Enum, is a config bug rejected at
// Add.

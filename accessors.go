...
// shared implementation used by operation handlers to read operation
// arguments.
//
// All read from `input map[string]any`, matching Handler.Execute's signature.
// Absent/nil keys return the provided default; the string, slice, and numeric
// variants also tolerate the shapes produced by JSON decoding (json.Number,
// float64, []any).
...
// emitted as JSON integers by a list operation round-trip into the string id a
// get/delete input schema expects.
...
// (e.g. "use the op default" on create, like a priority value that defaults to
// 10 when not supplied). It also tolerates a plain int that skipped the
// nullable coercion, for callers that build input directly.
